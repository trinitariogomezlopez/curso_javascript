...
El objeto `document` proporciona a los scripts el acceso a todos los elementos HTML dentro de una página. Además, este objeto forma parte además del objeto window, y puede ser accedido a través de la propiedad `window.document` o directamente `document` (ya que podemos omitir la referencia a la window actual).
